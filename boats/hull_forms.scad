...
module canoe_hull(){
    hull(){
        for(i=[0,1]) mirror([0,i,0]) translate([0,length/2,0]) {
            //prow
            translate([0,-prow_rad/2,prow_rad]) scale([.2,1,1]) sphere(r=prow_rad*1.25);
                
            for(j=[0,1]) mirror([j,0,0]) translate([width/2,-prow_rad-rad,rad]) {
                sphere(r=rad);
                translate([rad,-rad*2,prow_rad-rad]) sphere(r=rad);
            }
        }
    }
}
...
module canoe_mold(){
    ...
    difference(){
        intersection(){
            cube([in*1.75, in*6, in*1.5], center=true);
...
module catamaran_mold(){
    ...
    difference(){
        intersection(){
            ...
            cube([in*3.75, in*6, in*1.25], center=true);
        }
    ...
    difference(){
        intersection(){
            union(){
                ...
                for(i=[-5:1:5]) translate([0,i*3,in/2]) rotate([0,90,0]) cylinder(r=3, h=13, center=true, $fn=6);
            }
